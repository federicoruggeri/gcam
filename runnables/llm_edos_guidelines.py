    ...
    for item in guidelines:
        try:
            ...
            if parsed_item > 0 and parsed_item - 1 < num_labels:
                ...
                break   # we only pick the first guideline since it is multiclass
        except ValueError:
            continue
...
def get_true_labels(df, guidelines):
    y_true_classes = [1 if label == 'sexist' else 0 for label in test_data.label_sexist.values]
...
if __name__ == '__main__':
    ...
    y_true_classes, y_true_guidelines = get_true_labels(df=test_data, guidelines=guidelines)
    ...
    if not metrics_path.exists() or args.force:
        ...
        guideline_indexes = [f'G{idx + 1}' for idx in range(guidelines.shape[0])]
        ...
        prompt = [
            ...
            },
            {
                ...
                Respond only by choosing one index from {guideline_indexes} or respond with NO if the GUIDELINE SET does not relate to the input text. 
        ...
        prompts = [prompt.format(text=text, guideline_indexes=guideline_indexes, guideline=guidelines_str)
                   for text in texts]
    ...
    # guidelines
    metric_guidelines = F1Score(task='multiclass', average='macro', num_classes=num_labels)
    f1_guidelines = metric_guidelines(th.tensor(parsed_responses).argmax(dim=-1), th.tensor(y_true_guidelines).argmax(dim=-1))
