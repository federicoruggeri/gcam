...
if __name__ == '__main__':
    ...
    parser.add_argument('--category', '-c', default='CR', type=str, help='A | CH | CR | LTD | TER')
    ...
    prompt = [
        ...
        {
            'role': 'user',
            'content': """Your task is to classify input TEXT as containing an unfair clause for the user or client.
    ...
    test_data.to_csv(save_path.joinpath('response_classes_nog.csv'), index=False)

    metric = F1Score(task='multiclass', average='macro', num_classes=2)
    f1_score = metric(th.tensor(parsed_responses), th.tensor(y_true))
    print('F1: ', f1_score)
    np.save(save_path.joinpath('metrics_classes_nog.npy').as_posix(), f1_score)
