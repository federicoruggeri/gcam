...
from components.data_loader import EDOSLoader
from components.model import MultilabelModel
from components.processing import MultilabelCollator
from configurations.base import ConfigKey
from configurations.model import MultiLabelConfig
from modeling.discrete import MultiLabel
...
if __name__ == '__main__':
    ...
    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'edos', 'multilabel')
    ...
    config = MultiLabelConfig.from_config(key=ConfigKey(dataset='edos'))
    ...
    loader = EDOSLoader(data_dir=data_dir,
    ...
    collator = MultilabelCollator(padding_value=loader.tokenizer.pad_token_id)
    ...
    for seed in config.seeds:
        ...
        model = MultiLabel(hf_model_name=config.hf_model_name,
                           dropout_rate=config.dropout_rate,
                           freeze_hf=config.freeze_embeddings,
                           num_labels=loader.num_kb_labels)

        model = MultilabelModel(model=model,
        ...
        model = MultilabelModel.load_from_checkpoint(checkpoint_path=ckpt_filepath,
                                                     model=model.model,
                                                     val_metrics=MetricCollection({
                                                         'f1': F1Score(task='multiclass', average='macro',
