...
from components.model import DiscreteModel
from components.processing import DiscreteCollator
from configurations.base import ConfigKey
from configurations.model import DiscreteConfig
from modeling.discrete import Discrete
...
if __name__ == '__main__':
    ...
    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'edos', 'discrete')
    ...
    config = DiscreteConfig.from_config(key=ConfigKey(dataset='edos'))
    ...
    collator = DiscreteCollator(padding_value=loader.tokenizer.pad_token_id)
    ...
    for seed in config.seeds:
        ...
        model = Discrete(hf_model_name=config.hf_model_name,
                         dropout_rate=config.dropout_rate,
                         freeze_hf=config.freeze_embeddings,
                         num_classes=loader.num_classes)

        model = DiscreteModel(model=model,
                              num_classes=loader.num_classes,
        ...
        model = DiscreteModel.load_from_checkpoint(checkpoint_path=ckpt_filepath,
                                                   model=model.model,
                                                   val_metrics=MetricCollection({
